...
# FIX: Added 'id=' with a prefix to cache mounts as required by Docker BuildKit/Railway
RUN --mount=type=cache,id=spoticord-cache-cargo-registry,target=/usr/local/cargo/registry \
    --mount=type=cache,id=spoticord-cache-app-target,target=/app/target \
